...
# to do batch nomrlaisation:

# perl /mnt/shared/scratch/pthorpe/apps/trinityrnaseq-v2.9.0/Analysis/DifferentialExpression/remove_batch_effects_from_count_matrix.pl -m genes.counts.matrix -b samples_described.txt
...
*batch_eff_removal.matrix --samples_file samples_described.txt --method edgeR --output ${outdir}


# dont need to run this command if using TPM from slamon or kalisto. 
${trin_path}/run_TMM_normalization_write_FPKM_matrix.pl --matrix *batch_eff_removal.matrix --lengths genes_feature_lengths.txt

# QC steps

${trin_path}/PtR  --order_columns_by_samples_file  --matrix \
*batch_eff_removal.matrix --samples ${cuwodi}/samples_described.txt --CPM --log2 --min_rowSums 10 --compare_replicates

wait
${trin_path}/PtR  --order_columns_by_samples_file  --matrix \
*batch_eff_removal.matrix --min_rowSums 2 --samples ${cuwodi}/samples_described.txt --log2 --CPM --sample_cor_matrix

wait
${trin_path}/PtR --matrix *batch_eff_removal.matrix --pdf_width 14 --pdf_height 14 \
-s ${cuwodi}/samples_described.txt --min_rowSums 10 --log2 --CPM --center_rows --prin_comp 3

# does not need gene lenghts for this method 
### ${trin_path}/run_TMM_normalization_write_FPKM_matrix.pl --matrix *batch_eff_removal.matrix --just_do_TMM_scaling
...
deseq2="${trin_path}/run_DE_analysis.pl --matrix *batch_eff_removal.matrix --samples_file ${cuwodi}/samples_described.txt 
